...
namespace Plants_Vs_Zombies
{
    class Sparabrina : Pianta
    {
        ...
        public static IntRect l_sb = new IntRect(0, 0, 339, 170);
        ...
        public Sparabrina(int x, int y) : base(x, y)
        {
            ...
            Program.n_soli -= Program.piante[Program.lista_piante[3]].prezzo;
        ...
        private void Seme_On_Elapsed(object sender, ElapsedEventArgs e)
        {
            if (Spara())
                ...
                {
                    SemeBrina s = new SemeBrina(new Vector2f(X, Y), danno, 100, 7500);
                }
        }
        ...
        {
            L_SB.Position = new Vector2f(25, 65 * posizione + 10);
            if (selezionato)
                L_SB.Scale = new Vector2f(0.36f, 0.36f);
            else
                L_SB.Scale = new Vector2f(0.34f, 0.34f);
            Grafica.Finestra.Draw(L_SB);
        }
